...
module angle
    implicit none
    ! input
    integer, allocatable :: ids(:,:)  ! (nangle, 3)
    real(8), allocatable :: k(:)   ! (nangle)
    real(8), allocatable :: theteq(:)      ! (nangle)
    integer, allocatable :: to_ipair(:,:) ! (nangle, 3)
    ! output
    real(8) :: energy
    real(8), allocatable :: forces(:, :)   ! (natom, 3)
    real(8), allocatable :: tbforces(:, :) ! (ntbf, 3)
...
contains
    subroutine calculate()
        ...
        do iang=1, nangle
            itbf_ij = to_ipair(iang, 1) ! => ij
            itbf_ik = to_ipair(iang, 2) ! => ik
            itbf_jk = to_ipair(iang, 3) ! => jk
...
module dihedral
    implicit none
    ! input
    integer, allocatable :: ids(:,:)   ! (ntorsion, 4)
    integer, allocatable :: per(:)      ! (ntorsion) dihedral periodicity
    real(8), allocatable :: phi_k(:)   ! (ntorsion) force constant (kcal/mol)
    real(8), allocatable :: phase(:) ! (ntorsion) dihedral phase in radians
    integer, allocatable :: to_ipair(:,:) ! (ntorsion, 6)
    ! output
    real(8) :: energy
    real(8), allocatable :: forces(:, :)   ! (natom, 3)
...
contains
    subroutine calculate()
        ...
        do itor=1, ntorsion
            ...
            if( dabs( phi_k(itor) ) <= eps ) cycle

            itbf_ij = to_ipair(itor, 1) ! => ij
            itbf_ik = to_ipair(itor, 2) ! => ik
            itbf_il = to_ipair(itor, 3) ! => il
            itbf_jk = to_ipair(itor, 4) ! => jk
            itbf_jl = to_ipair(itor, 5) ! => jl
            itbf_kl = to_ipair(itor, 6) ! => kl
            ...
            ! calculate energy
            new_phi = nfreq*phi + phase(itor)
            ene = kt * (1.0d0 + cos(new_phi))
            energy = energy + ene
...
module improper
    ...
    ! input
    integer, allocatable :: ids(:,:)   ! (ntorsion, 4)
    real(8), allocatable :: psi_k(:)   ! (ntorsion) Force constant in kcal/mol/radians^2
    real(8), allocatable :: psi_eq(:) ! (ntorsion) Equilibrium torsion angle in radians
    integer, allocatable :: to_ipair(:,:) ! (ntorsion, 6)
    ! output
    real(8) :: energy
    real(8), allocatable :: forces(:, :)   ! (natom, 3)
...
contains
    subroutine calculate(interact_table, ninteract)
        ...
        ! gained from Amber definition. ! e^2 / (4 pi eps_0) [kcal/mol * A/eV^2]
...
module coulomb14
    ...
    ! input
    real(8), allocatable :: charges(:)     ! (natom)
    integer, allocatable :: to_ipair(:) ! (n14)
    ! output
    real(8) :: energy                      ! enegry
    real(8), allocatable :: forces(:, :)   ! (natom, 3)
    real(8), allocatable :: tbforces(:, :) ! (ntbf, 3)
